...
def validate_readonly_sql(
    ...
    """Raises InumiError(SECURITY_BLOCKED | INVALID_ARGUMENTS) or returns a
    validated, row-limited, single SELECT statement's AST-derived summary."""
    if len(sql) > _MAX_STATEMENT_LENGTH:
        raise InumiError(FailureCode.INVALID_ARGUMENTS, "SQL statement is too long.")

    try:
        statements = sqlglot.parse(sql, read=dialect)
    except Exception as exc:  # noqa: BLE001 — any parser error is a rejection, not a crash
        raise InumiError(FailureCode.INVALID_ARGUMENTS, f"Could not parse SQL: {exc}") from exc
    ...
    if statement.args.get("into") is not None:
        raise InumiError(FailureCode.SECURITY_BLOCKED, "SELECT ... INTO is not permitted.")
    ...
    for node in statement.walk():
        ...
        if any(bad in text for bad in _DANGEROUS_FUNCTIONS):
            raise InumiError(FailureCode.SECURITY_BLOCKED, "Statement references a disallowed construct.")
