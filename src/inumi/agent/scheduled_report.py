...
from inumi.agent.orchestrator import AgentOrchestrator, ScheduledSummary
...
DAILY_DIGEST_JOB_ID = "inumi_daily_digest"
...
class ChannelsDigestPublisher:
    ...
    async def publish(self, *, channel_id: str, text: str) -> None:
        token = self._issuer.issue(service_name="agent", audience="inumi-channels")
...
def _server_block(summary: ScheduledSummary) -> str:
    ...
    if summary.dropped_proposals:
        ...
        # it can't be misread as "Inumi did something" — see
        ...
        lines.append(
            f"(Inumi would have proposed {proposed} here. A scheduled sweep never "
...
class DailyDigestRunner:
    ...
    async def run_once(self) -> str | None:
        ...
        try:
            ...
            digest = (
                f"Inumi daily health digest — {generated_at.strftime('%Y-%m-%d %H:%M')} UTC"
...
def schedule_daily_digest(
    ...
    scheduler.add_job(
        ...
        name="Inumi daily multi-server health digest",
        coalesce=True,
