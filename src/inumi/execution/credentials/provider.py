...
`InumiError(FailureCode.DEPENDENCY_UNAVAILABLE, ...)`. A raw SDK exception
...
def _sdk_missing_error(provider_name: str, package: str, extra: str, exc: Exception) -> InumiError:
    return InumiError(
        FailureCode.DEPENDENCY_UNAVAILABLE,
...
def _backend_error(provider_name: str, database_id: str, exc: Exception) -> InumiError:
...
def _malformed_secret_error(provider_name: str, database_id: str, reason: str) -> InumiError:
...
class VaultCredentialProvider(_UnconfiguredSecretsManagerProvider):
    ...
    **Path convention.** The credential for server id `<id>` is read from
    ...
    `secret/inumi/db/<id>`, which is `secret/data/inumi/db/<id>` in Vault's
    ...
    **Secret contents.** The KV secret's `data` is the JSON object
    ...
    ```
    vault kv put secret/inumi/db/pg-prod-01 \\
        host=db-prod-01.internal port=5432 username=inumi_diag \\
...
class AWSSecretsManagerCredentialProvider(_UnconfiguredSecretsManagerProvider):
    ...
    **Name convention.** The credential for server id `<id>` is the secret
    named `<name_prefix>/<id>` — by default `inumi/db/<id>`. A full ARN
    ...
    **Secret contents.** The secret's `SecretString` is the JSON object
    ...
    ```
    aws secretsmanager create-secret --name inumi/db/pg-prod-01 \\
        --secret-string '{"host":"...","port":5432,"username":"inumi_diag",
                          "password":"...","database":"postgres"}'
    ```
    ...
    **Auth.** The standard boto3 credential chain (instance/task role,
    ...
    `secretsmanager:GetSecretValue` on `inumi/db/*` and nothing else.
    """

    name = "aws_secrets_manager"

    def __init__(self, region: str = "", *, name_prefix: str = "inumi/db", client: Any = None):
        self._region = region
...
class AzureKeyVaultCredentialProvider(_UnconfiguredSecretsManagerProvider):
    ...
    **Name convention.** The credential for server id `<id>` is the secret
    named `<name_prefix>-<id>` — by default `inumi-db-<id>`. Key Vault
    ...
    **Secret contents.** The secret's *value* is the JSON object documented
    ...
    ```
    az keyvault secret set --vault-name inumi-kv --name inumi-db-pg-prod-01 \\
        --value '{"host":"...","port":5432,"username":"inumi_diag",
                  "password":"...","database":"postgres"}'
    ```
    ...
    **Auth.** `DefaultAzureCredential` — managed identity in Azure,
    ...
    def __init__(self, vault_url: str = "", *, name_prefix: str = "inumi-db", client: Any = None):
...
class GCPSecretManagerCredentialProvider(_UnconfiguredSecretsManagerProvider):
    ...
    **Name convention.** The credential for server id `<id>` is read from
    ...
    by default the `latest` version of `inumi-db-<id>`. GCP secret ids
    ...
    **Secret contents.** The version's payload is the JSON object documented
    ...
    ```
    printf '{"host":"...","port":5432,"username":"inumi_diag",
             "password":"...","database":"postgres"}' |
      gcloud secrets create inumi-db-pg-prod-01 --data-file=-
    ```
